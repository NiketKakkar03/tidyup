...
fn human_action_record_status(record: &ActionResultRecord) -> String {
    match record.status_code.as_str() {
        ...
        "skipped" => record.reason_code.as_deref().map_or_else(
            || "skipped".to_owned(),
            |reason| human_validation_reason(reason).to_owned(),
        ),
...
mod tests {
    ...
    fn help_is_returned_without_arguments() {
        let output = run(&[]).expect("usage should render");
    ...
    fn unknown_command_is_rejected() {
        let error = run(&["organize".to_owned()]).expect_err("command should fail");
